...
def train(args, model: Model, dataset, *arg):
    ...
    if 'wsl' in args.debug or not enable_phase1:
        # [关键保留] 传递 clip_model 用于特征提取
        cma.extract(args, model, dataset, clip_model=model.clip_model)
    ...
    pbar = tqdm(zip(rgb_loader, ir_loader), total=iters, desc=f"Epoch {epoch}", leave=False)
